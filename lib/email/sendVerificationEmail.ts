...
export async function sendVerificationEmail({ email, name, verificationUrl }: Props): Promise<void> {
...
<head>
  ...
  <title>Verify your Gunimi account</title>
  <style>
            ...
            <td class="card" style="background-color:#0A0E17;border:1px solid rgba(255,255,255,0.06);border-radius:20px;overflow:hidden;">
              ...
              <div class="content-pad" style="padding:40px 40px;">
                ...
                <table role="presentation" cellspacing="0" cellpadding="0" border="0">
                  <tr>
                    <td style="background-color:rgba(109,91,255,0.1);border:1px solid rgba(109,91,255,0.2);border-radius:20px;padding:4px 12px;">
                      <span style="font-size:11px;font-weight:600;color:#8B7DFF;letter-spacing:0.12em;text-transform:uppercase;">Verify your account</span>
                    </td>
                  </tr>
                </table>
                ...
                <h1 class="heading" style="margin:24px 0 12px;font-size:26px;font-weight:700;color:#F7F8FC;letter-spacing:-0.03em;line-height:1.2;">
                  Hi ${displayName},<br />one more step.
                </h1>

                <!-- Body -->
                <p class="body-text" style="margin:0 0 28px;font-size:15px;line-height:1.7;color:#9AA3B2;">
                  Click the button below to verify your email and activate your <strong style="color:#F7F8FC;">Gunimi</strong> account.<br /><br />
                ...
                <table role="presentation" cellspacing="0" cellpadding="0" border="0">
                  <tr>
                    <td style="border-radius:12px;background:linear-gradient(135deg,#6D5BFF,#5B4AE8);box-shadow:0 0 24px rgba(109,91,255,0.35);">
                      <a href="${verificationUrl}" target="_blank" style="display:inline-block;padding:14px 28px;font-size:14px;font-weight:600;color:#ffffff;text-decoration:none;border-radius:12px;letter-spacing:-0.01em;">
                ...
                <p style="margin:0;font-size:12px;color:rgba(154,163,178,0.5);line-height:1.5;">
                  This email was sent to <strong>${email}</strong>. If you didn't create a Gunimi account, you can safely ignore this email.
                </p>

                <p style="margin:16px 0 0;font-size:11px;color:rgba(154,163,178,0.35);word-break:break-all;line-height:1.4;">
                  If the button doesn't work, copy this link: <a href="${verificationUrl}" style="color:rgba(109,91,255,0.6);text-decoration:none;">${verificationUrl}</a>
                </p>
              </div>
            </td>
          </tr>
...
Please verify your Gunimi account by clicking the link below:
...
${process.env.NEXT_PUBLIC_APP_URL}`;
  ...
  await sendEmail({
    ...
    to: email,
    subject: "Verify your Gunimi account",
    html,
    text,
  });
}
