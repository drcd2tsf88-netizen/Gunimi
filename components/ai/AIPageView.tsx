...
const MODULES = [
  { key: "chat", icon: MessageSquare, alpha: true },
  { key: "automation", icon: Zap, alpha: false },
  { key: "insights", icon: BarChart2, alpha: false },
  { key: "knowledge", icon: BookOpen, alpha: false },
] as const;
    ...
    <div className="space-y-10">
      ...
      <OrbitSection>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {MODULES.map(({ key, icon: Icon, alpha }) => (
            <OrbitCard key={key} className="p-6">
              <div className="flex items-start justify-between gap-4">
                ...
                {alpha ? (
                  <span className="shrink-0 rounded-full border border-violet-500/20 bg-violet-500/10 px-2.5 py-0.5 text-[10px] font-medium uppercase tracking-wide text-violet-300">
                    {t("alphaBadge")}
                  </span>
                ) : (
