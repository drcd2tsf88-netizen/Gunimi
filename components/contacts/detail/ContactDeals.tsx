...
export default function ContactDeals({ deals }: Props) {
                ...
                <OrbitCard className="p-5">
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      ...
                      {deal.description && (
                        <p className="mt-3 text-sm text-white/60">{deal.description}</p>
                      )}
                    </div>
                  ...
                  <div className="mt-5 flex flex-wrap gap-2">
                    <div
                      className={`rounded-full border px-3 py-1 text-xs ${getStageStyles(deal.stage)}`}
                    >
                      {tDeals(deal.stage as Parameters<typeof tDeals>[0])}
                    </div>
                    {deal.probability != null && (
                      <div className="rounded-full border border-white/10 bg-white/[0.03] px-3 py-1 text-xs">
                        {deal.probability}%
                      </div>
                    )}
                  </div>
                  ...
                  <div className="mt-5 grid gap-4 md:grid-cols-3">
                    ...
                    <div>
                      <p className="text-xs text-white/40">{tDeals("owner")}</p>
                      <p className="mt-1 text-sm">
                        {deal.owner?.full_name || tDeals("unassigned")}
                      </p>
                    </div>
                    <div>
                      <p className="text-xs text-white/40">{tDeals("expectedClose")}</p>
                      <p className="mt-1 text-sm">
